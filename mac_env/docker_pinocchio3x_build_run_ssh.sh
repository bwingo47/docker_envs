...
GIT_LOGIN_EMAIL=qikai.huang@inria.fr
...
docker build -f $DOCKERFILE_NAME \
  ...
  --build-arg SSH_PRV_KEY="$(cat $HOME/.ssh/id_ed25519_inria_gitlab)" \
  --build-arg SSH_PUB_KEY="$(cat $HOME/.ssh/id_ed25519_inria_gitlab.pub)" \
